...
UPDATE useraccounts as u SET u.lastLoginDate=NOW() WHERE username=LOWER(myuser) AND combinedHash=WSService_MakeHash(myhash, salt);
SELECT * FROM useraccounts WHERE username=LOWER(myuser) AND combinedHash=WSService_MakeHash(myhash, salt);
